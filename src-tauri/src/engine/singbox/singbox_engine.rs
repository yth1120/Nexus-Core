...
impl Engine for SingBoxEngine {
    ...
    async fn start(&self) -> AppResult<()> {
        ...
        if config_path.is_empty() {
            return Err(AppError::Internal("sing-box config path not set".into()));
        }
        let config = config_generator::generate(&Default::default(), &[], &[])?;
        std::fs::write(&config_path, &config)
            .map_err(|e| AppError::Io(format!("write config: {e}")))?;
        let pc = ProcessConfig::new("sing-box", &config_path);
    ...
    async fn health_check(&self) -> AppResult<()> {
            ...
            .is_some_and(|h| h.is_running())
        {
            ...
            Err(AppError::Internal("sing-box not running".into()))
        }
    }
